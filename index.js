...
client.on('messageCreate', async message => {
  ...
  // منع شخص من دخول الرومات الصوتية (BlackVoice)
  if (command === '!blackvoice' || command === '!bv') {
    ...
    if (!/^\d+$/.test(userId || '')) {
      return message.reply(
        ':warning: يرجى كتابة الـ ID الصحيح أو المنشن. مثال: `!blackvoice 123456789`'
      );
    }
  ...
  // إزالة شخص من قائمة منع الفويس (UnblackVoice)
